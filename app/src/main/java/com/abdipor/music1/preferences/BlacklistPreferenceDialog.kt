...
import com.abdipor.music1.dialogs.BlacklistFolderChooserDialog
import com.abdipor.music1.extensions.colorControlNormal
import com.abdipor.music1.providers.BlacklistStore
        ...
        return MaterialDialog(requireContext()).show {
            ...
            neutralButton(text = getString(R.string.clear_action)) {
                MaterialDialog(requireContext()).show {
                    title(com.abdipor.music1.R.string.clear_blacklist)
                    message(com.abdipor.music1.R.string.do_you_want_to_clear_the_blacklist)
                    cornerRadius(PreferenceUtil.getInstance(requireContext()).dialogCorner)
                    positiveButton(com.abdipor.music1.R.string.clear_action) {
            ...
            listItems(items = paths, waitForPositiveButton = false) { _, _, text ->
                MaterialDialog(requireContext()).show {
                    cornerRadius(PreferenceUtil.getInstance(requireContext()).dialogCorner)
                    title(com.abdipor.music1.R.string.remove_from_blacklist)
                    message(
                    ...
                    positiveButton(com.abdipor.music1.R.string.remove_action) {
